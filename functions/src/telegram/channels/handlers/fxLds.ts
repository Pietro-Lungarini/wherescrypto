...
const handleUpdate = async (
  ...
  const text = msg.message.toLowerCase();
  if (!original) return;
  const docRef = firestore().doc(fxLdsDbPath(original));
  const document = await docRef.get();
  if (!document.exists) return;

  const include = (word: string) => text.includes(word);

  if (include('move sl') || include('modifiez sl à')) {
    const moveSl = () => {
      const text1 = text + '\n';
      const rStr = 'sl';
  ...
  if (include('secure') || include('sécurisez')) {
    if (include('partial close')) {
      return {
        ...(document.data() as ForexSignal),
        action: ['break-even', 'partial-close'],
        actionOptions: {
          closeQty: 0.5,
        },
      };
